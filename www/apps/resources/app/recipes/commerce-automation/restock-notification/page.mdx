...
The second step you'll create will validate that the variant is actually out of stock in the customer's sales channel.

Before implementing the step, you'll add a utility function that checks whether the variant is out of stock in the specified sales channel. So, create the file `src/workflows/utils/is-variant-in-stock.ts` with the following content:
...
```ts title="src/workflows/utils/is-variant-in-stock.ts"
import { MedusaError } from "@medusajs/framework/utils"
...
You create an `isVariantInStock` function that accepts two parameters:

- An object having the following properties:
    - `variant_id`: The ID of the variant to check if it's in stock.
    - `sales_channel_id`: The ID of the customer's sales channel.
- A step context object that a step function receives as a second parameter. This is necessary to later execute a Medusa workflow that checks the variant's inventory.

In the function, you resolve Query from the Medusa container and retrieve the variant using Query's `graph` method. Specifically retrieving inventory-related fields and relations.

Next, to check if the variant is in stock, replace the `TODO` with the following:
...
You create an `isInStock` variable whose value is `false`, then execute Medusa's `confirmVariantInventoryWorkflow` to check whether the variant is in stock. Since you're executing a workflow within a function that will be used in a step, you must pass the step's context in the workflow's `run` method.

The `confirmVariantInventoryWorkflow` accepts as an input an object having the following properties:

- `variants`: The variants to check their inventory availability. You pass the data returned by Query.
- `sales_channel_id`: The ID of the sales channel to check the inventory in. You pass the sales channel ID from the function's first parameter.
- `items`: An array of objects, each having the ID of the variant and the quantity to check if it's available. Since you're only checking if the variant is in stock, you pass `1` for the quantity.

<Note title="Tip">

Refer to the [confirmVariantInventoryWorkflow's reference](/references/medusa-workflows/confirmVariantInventoryWorkflow) for more details on its input and steps.

</Note>

If `confirmVariantInventoryWorkflow` throws an error, the variant is out of stock. So, you wrap the workflow in a try-catch block, and only change the value of `isInStock` if the workflow doesn't throw an error. Finally, you return `isInStock`.
...
This step accepts the ID of the variant and the ID of the customer's sales channel. In the step, you use the `isVariantInStock` utility to check whether the variant is out-of-stock. If not, you throw an error, stopping the workflow's execution.
...
export const subscriptionWorkflowHighlights = [
  ["17", "createWorkflow", "Create a workflow."],
  ["24", "getEmailStep", "Get the subscriber's email."],
  ["26", "validateVariantOutOfStockStep", "Validate that the variant is out of stock in the specified sales channel."],
  ["31", "useQueryGraphStep", "Get the restock subscription to check if it already exists."],
  ["41", "when", "Perform an action if the restock subscription doesn't exist."],
  ["45", "createRestockSubscriptionStep", "Create the restock subscription if it doesn't exist."],
  ["53", "when", "Perform an action if the restock subscription exists."],
  ["57", "updateRestockSubscriptionStep", "Update the restock subscription if it exists."],
  ["64", "useQueryGraphStep", "Retrieve the restock subscription again to return it."],
  ["74", "WorkflowResponse", "Return the restock subscription."]
]
...
- Use the [when-then](!docs!/learn/advanced-development/workflows/conditions) from the Workflows SDK to perform an action if a condition is met.
...
A workflow's constructor function has some constraints in implementation, which is why you need to use `when-then` to perform steps based on a condition. Learn more about these constraints in [this documentation](!docs!/learn/advanced-development/workflows/constructor-constraints).
...
In this step, you loop over the restock subscriptions and use the `isVariantInStock` utility to check whether a subscription's variant is out-of-stock in the sales channel.
