...
- Implement the data models to subscribe for variant restocking.
...
In this step, you'll create a Restock Module that adds custom data models for subscriptions to restock notifications. In later steps, you'll store customer subscriptions in these data models.
...
4. `subscribers`: A many-to-many relation to the customers subscribed to the variant's restock notification in their sales channel. You'll create the associated data model next.
...
You also define a unique index on the `variant_id` and `sales_channel_id` properties using the `indexes` method.
...
To store the customers subscribed to a variant's restock notification in their sales channel, create the file `src/modules/restock/models/restock-subscriber.ts` with the following content:

![The directory structure of the Restock Module after adding this model.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733224727/Medusa%20Resources/restock-dir-overview-3_ycmm6a.jpg)

```ts title="src/modules/restock/models/restock-subscriber.ts"
import { model } from "@medusajs/framework/utils";
...
You define the `RestockSubscriber` data model that has the following properies:

- `email`: The customer's email used as a primary key.
- `customer_id`: The customer's ID in Medusa. This is nullable in case the customer is a guest.
- `subscriptions`: A many-to-many relation to the `RestockSubscription` data model, indicating the customer's restock subscriptions.
...
In this section, you'll create the Restock Module's service. Create the file `src/modules/resend/service.ts` with the following content:
...
So, the `RestockModuleService` class now has methods like `createRestockSubscriptions` and `retrieveRestockSubscriber`.
...
You'll use this service in a later method to store and manage restock subscriptions and subscribers.
...
The tables of the Restock Module's data models are not created in the database.
    ...
    steps: [
      {
        type: "step",
        name: "getEmailStep",
        description: "Get the subscription email, either from the specified customer or from the input.",
        depth: 1,
        link: "#getemailstep"
      },
      {
      ...
      {
        type: "step",
        name: "createOrGetRestockSubscriptionsStep",
        description: "Create or retrieve existing subscription for the variant and customer's sales channel.",
        depth: 1,
        link: "#createorgetrestocksubscriptionsstep"
      },
      {
        type: "step",
        name: "addRestockSubscriberStep",
        description: "Add the customer as a subscriber to the restock subscription of the variant in the customer's sales channel.",
        depth: 1,
        link: "#addrestocksubscriberstep"
      }
    ]
  }}
/>

You'll implement these steps in order, then create the workflow.

### getEmailStep

Both guest and registered customers can subscribe to a variant's restock notification. So, the first step you'll create returns the email to use in the subscription, based on whether an email or a customer ID is passed in the workflow's input.

To create the step, create the file `src/workflows/create-restock-subscription/steps/get-email.ts` with the following content:

![The Medusa application's directory structure after adding the step.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733226166/Medusa%20Resources/restock-dir-overview-8_eg2bt0.jpg)

export const getEmailHighlights = [
  ["9", "createStep", "Create a step."],
  ["11", "container", "The Medusa container to resolve resources."],
  ["20", "query", "Resolve Query from the Medusa container."],
  ["23", "graph", "Retrieve the customer by its ID to use its email."],
  ["34", "StepResponse(returnedEmail)", "Return the email."]
]

```ts title="src/workflows/create-restock-subscription/steps/get-email.ts" highlights={getEmailHighlights}
...
You define the `getEmailStep` using `createStep` from the Workflows SDK. `createStep` accepts two parameters:
...
The Medusa container is accessible by all customizations, such as workflows and scheduled jobs, except for modules. Each module has its own container with framework tools like the [Logger utility](!docs!/learn/debugging-and-testing/logging).

</Note>

In the step function, you first check that at least an email or a customer is passed and throw an error otherwise. Then, you resolve [Query](!docs!/learn/advanced-development/module-links/query), a tool to retrieve data across modules, from the container.

If the email isn't passed in the input, you use Query's `graph` method to retrieve the customer's email, filtering by the customer ID passed in the step input.

The step returns an instance of `StepResponse`, which must be returned by any step. It accepts as a parameter the data to return to the workflow that executed this step, which is the customer's email.

### validateVariantOutOfStockStep

The second step you'll create will validate that the variant is actually out of stock in the customer's sales channels.

Before implementing the step, you'll add a utility function to chech whethere there are locations in the customer's sales channels where a variant is out of stock. So, create the file `src/workflows/utils/has-out-of-stock-locations.ts` with the following content:

![The Medusa application's directory structure after adding the utility.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733226495/Medusa%20Resources/restock-dir-overview-9_xpegq1.jpg)

```ts title="src/workflows/utils/has-out-of-stock-locations.ts"
...
export const hasOutOfStockLocations = async (
  ...
  // TODO retrieve variant's details
}
```

You create a `hasOutOfStockLocations` function that accepts two parameters:

- An object having the following properties:
    - `variant_id`: The ID of the variant to retrieve its out-of-stock locations.
    - `sales_channel_ids`: The IDs of the customer's sales channels. A customer can be browsing your store from multiple sales channels.
- The Medusa container to resolve resources from.

In the function, you first validate that the `sales_channel_ids` isn't empty. Then, you resolve Query and the [Inventory Module](../../../commerce-modules/inventory/page.mdx)'s service from the Medusa container.

Next, you'll retrieve the variant's inventory details. So, replace the `TODO` with the following:
...
// TODO handle variant data
```

You use Query's `graph` method to retrieve the product variant's fields and relations, specifically inventory related. You also retrieve the `manage_inventory` field, which is a boolean that indicates whether Medusa manages the variant's inventory quantity.

If `manage_inventory` is disabled, the variant is always considered in stock. So, replace the `TODO` with the following:
...
// TODO get variant's out-of-stock locations
```

If `manage_inventory` is disabled, you return `hasOutOfStock` which is still `false`, indicating that there are no locations where the variant is out-of-stock.

Next, you'll handle variants where Medusa manages their inventory details. You first need to filter out from the variant's inventory items those that aren't in locations associated with the customer's sales channels. So, replace the `TODO` with the following:
...
// TODO check inventory in sales channel's locations
```

You use `deepFlatMap` which is a Medusa utility that makes nested arrays flat, allowing you to access the items easily. The function accepts three parameters:

1. The object to flatten, which is the variant.
2. The path to flatten, which is the path from the variant's inventory to the sales channels associated with the variant's stock locations.
3. A callback function that accepts the flat object as a parameter to perform operations on it.

In the callback function, you filter out sales channels that don't belong to the customer's sales channels, and build a map between the variant's inventory items and the sales channel's locations.

Lastly, you'll check the inventory quantity of the variant in the sales channel's locations using the `inventoriesToCheck` object. So, replace the `TODO` with the following:

```ts title="src/workflows/utils/get-out-of-stock-locations.ts"
await promiseAll(
...
You loop over the `inventoriesToCheck` object and use the `confirmInventory` method of the Inventory Module's service to check that at least one inventory item is in stock for each location. If not, you set `hasOutOfStock` to `true`. Finally, you return the `hasOutOfStock` variable.

<Note title="Tip">

When looping over promises, use Medusa's `promiseAll` utility which accepts an array of promises and ensures that all transactions within these promises are rolled back in case an error occurs.

</Note>

You'll use this utility in the `validateVariantOutOfStockStep`. Create the file `src/workflows/create-restock-subscription/steps/validate-variant-out-of-stock.ts` with the following content:
...
This step accepts the ID of the variant and the IDs of the customer's sales channels. In the step, you use the `hasOutOfStockLocations` utility to check whether the variant is out-of-stock. If not, you throw an error, stopping the workflow's execution.

### createOrGetRestockSubscriptionsStep

The third step of the workflow is to either create restock subscriptions for the specified variant and sales channels, or retrieve existing ones. Existing subscriptions are due to another customer subscribing to the variant's restock notification in the same sales channel(s).

Create the file `src/workflows/create-restock-subscription/steps/create-or-get-restock-subscriptions.ts` with the following content:
...
export const createRSHighlights = [
  ...
  ["20", "listRestockSubscriptions", "Use the service's generated method to retrieve restock subscriptions."],
  ["25", "toCreate", "The data to create restock subscriptions that don't exist."],
  ["34", "createRestockSubscriptions", "Use the service's generated method to create restock subscriptions."]
]

```ts title="src/workflows/create-restock-subscription/steps/create-or-get-restock-subscriptions.ts" highlights={createRSHighlights}
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
...
export const createOrGetRestockSubscriptionsStep = createStep(
  ...
  async ({ 
    ...
    const restockSubscriptions = await restockModuleService
    ...
    const toCreate = sales_channel_ids.filter(
      (salesChannelId) => !restockSubscriptions.some(
        (rs) => rs.sales_channel_id === salesChannelId
      )
    ).map((salesChannelId) => ({
      variant_id,
      sales_channel_id: salesChannelId
    }))
    const created = await restockModuleService
      .createRestockSubscriptions(toCreate)
...
Then, you use the service's `listRestockSubscriptions` method, which was generated by `MedusaService`, to retrieve existing restock subscriptions of the variant with each specified sales channel. After that, you create the non-existing restock subscriptions using the service's generate `createRestockSubscriptions` method.
...
Finally, you return the list of restock subscriptions (created and existing) by passing it as a first parameter to `StepResponse`. The second parameter is data passed to the compensation function, which you'll learn about next.
...
Since the `createOrGetRestockSubscriptionsStep` creates some restock subscriptions, you'll undo that in the compensation function. To add a compensation function, pass it as a third parameter to `createStep`:

```ts title="src/workflows/create-restock-subscription/steps/create-or-get-restock-subscriptions.ts"
export const createOrGetRestockSubscriptionsStep = createStep(
  // ...
...
1. The second parameter of `StepResponse`, which is the created restock subscriptions.
...
In the compensation function, you resolve the Restock Module's service from the container, then delete the created subscriptions using the generated `deleteRestockSubscriptions`.

### addRestockSubscriberStep

The last step of the workflow creates a restock subscriber for the customer and attaches it to the restock subscriptions retrieved from the first step.

Create the file `src/workflows/create-restock-subscription/steps/add-restock-subscribers.ts` with the following content:
...
```ts title="src/workflows/create-restock-subscription/steps/add-restock-subscribers.ts"
import { createStep, StepResponse } from "@medusajs/framework/workflows-sdk"
import { RESTOCK_MODULE } from "../../../modules/restock"
import RestockModuleService from "../../../modules/restock/service"
...
export const addRestockSubscriberStep = createStep(
  ...
  }: AddRestockSubscriberStepInput, { container }) => {
    const restockModuleService: RestockModuleService = container.resolve(
      RESTOCK_MODULE
    )

    const restockSubscribers = await restockModuleService
      .listRestockSubscribers({
        email
      }, {
        relations: ["subscriptions"]
      })
    let restockSubscriber = restockSubscribers.length ? 
      restockSubscribers[0] : undefined
    const changes = {
      operation: restockSubscriber === undefined ? "created" : "updated",
      data: restockSubscriber
    }

    // TODO create or update the restock subscriber
  },
)
```

In the step, you resolve the Restock Module's service and use its generated `listRestockSubscribers` to retrieve the subscriber by its email. You also pass as a second parameter a `relations` property, indicating that the subscriber's subscriptions should be expanded.

<Note>

Learn more about the generated `list` method in [this reference](../../../service-factory-reference/methods/list/page.mdx).

</Note>

You then create a `changes` variable to store the details of the changes made by the step. You'll later pass this variable to the compensation function.

Next, replace the `TODO` with the following to create or update the restock subscriber:

```ts title="src/workflows/create-restock-subscription/steps/add-restock-subscribers.ts"
if (restockSubscriber) {
  const customerId = customer_id && 
    restockSubscriber.customer_id !== customer_id ? 
    customer_id : restockSubscriber.customer_id
  
  const subscriptionIds = [
    ...(restockSubscriber.subscriptions.map(
...
If the restock subscriber was already in the database, you update its list of subscriptions using the generated `updateRestockSubscribers` method. When updating a many-to-many relation, you must pass the ID of existing associations with the IDs of the new ones.

<Note>

Learn more about managing many-to-many relations in [this documentation](!docs!/learn/advanced-development/data-models/manage-relationships#manage-many-to-many-relationship).

</Note>

If the restock subscriber isn't in the database, you create a new one using the `createRestockSubscribers` generated method. Finally, you return the subscriber in the `StepResponse`'s first parameter, and the `changes` object as a second parameter to be passed to the compensation function.

#### Add Compensation Function

Using the `changes` variable in the compensation function, you'll either revert the update made on the restock subscriber, or delete it if it was created.

Pass a compensation function as a third parameter to `createStep`:
...
In the compensation function, you resolve the Restock Module's service from the container. Then, if `changes.operation` is created, you delete the created subscriber using the `deleteRestockSubscribers` generated method of the service. Otherwise, you revert the subscriber to its old data using the generated `updateRestockSubscribers` method.
...
export const subscriptionWorkflowHighlights = [
  ["16", "createWorkflow", "Create a workflow."],
  ["19", "getEmailStep", "Get the subscriber's email."],
  ["21", "validateVariantOutOfStockStep", "Validate that the variant is out of stock in the specified sales channels."],
  ["26", "createOrGetRestockSubscriptionsStep", "Create or retrieve the restock subscriptions in the database."],
  ["31", "restockSubscriptionIds", "Create a variable of subscription IDs."],
  ["39", "addRestockSubscriberStep", "Create or update the restock subscriber."],
  ["45", "WorkflowResponse", "Return an object of created or updated restock subscriptions and subscribers."]
]

```ts title="src/workflows/create-restock-subscription/index.ts" highlights={subscriptionWorkflowHighlights}
...
export const createRestockSubscriptionWorkflow = createWorkflow(
  ...
  (input: CreateRestockSubscriptionWorkflowInput) => {
    ...
    const restockSubscriptionIds = transform({
      ...
      return data.restockSubscriptions.map(
        (restockSubscription) => restockSubscription.id
      )
    })
...
- `getEmailStep` to retrieve the subscriber's email.
- `validateVariantOutOfStockStep` to validate that the variant is out of stock in the specified sales channels. If not, an error is thrown, halting the workflow's execution.
- `createOrGetRestockSubscriptionsStep` to create or retrieve the restock subscriptions for the variant and sales channels.
- Before proceeding to the next step, you use [transform](!docs!/learn/advanced-development/workflows/variable-manipulation) from the Workflows SDK to create a variable holding an array of restock subscription IDs. You can only manipulate data directly in a workflow using `transform`.
- `addRestockSubscriberStep` to create or update a restock subscriber of the customer.

<Note title="Tip">

A workflow's constructor function has some constraints in implementation, which is why you need to use `transform`. Learn more about these constraints in [this documentation](!docs!/learn/advanced-development/workflows/constructor-constraints).

</Note>

Workflows must return an instance of `WorkflowResponse`, passing as a parameter the data to return to the workflow's executor. The workflow returns an object holding the restock subscriptions and subscriber.
...
The API route will accept the variant ID and the customer email as request body parameters. So, you must create a schema to validate the request body.

In Medusa, you create validation schemas using [Zod](https://zod.dev/) in a `validators.ts` file under the `src/api` directory. So, create the file `src/api/store/restock-subscriptions/validators.ts` with the following content:
...
export const routeHighlights = [
  ["13", "POST", "Define a `POST` API route"],
  ["14", "req", "The request's details."],
  ["15", "res", "The object to manipulate and return a response."],
  ["18", "run", "Execute the `createRestockSubscriptionWorkflow`."],
  ["19", "input", "Pass input to the workflow."],
  ["20", "validatedBody", "Access body parameters in `validatedBody`."],
  ["21", "publishable_key_context", "Access the customer's sales channels based on the specified publishable API key."],
  ["24", "customer_id", "If the customer is logged in, their ID is stored in `req.auth_context.actor_id`."],
  ["29", "sendStatus", "Return a response with `201` status code."]
]
...
import { MedusaError } from "@medusajs/framework/utils"
import { 
  createRestockSubscriptionWorkflow
} from "../../../workflows/create-restock-subscription"
...
In the function, you execute the `createRestockSubscriptionWorkflow` by invoking it, passing it the Medusa container which is stored in the `scope` property of the request object, and invoking its `run` method.
...
2. `sales_channel_ids`: The IDs of the customer's sales channels. Every request sent to an API route under the `/store` path must have a [publishable API key](!api!/store#publishable-api-key) passed in the request's header. This API key is created in the Medusa Admin and associated with sales channels. Then, you can access the sales channel associated with the API key passed to a request in the `publishable_key_context.sales_channel_ids` property of the request object.
3. `customer`: The subscriber customer's details:
    - `email`: The email passed in the request body, if available.
    - `customer_id`: The ID of the customer if they're authenticated. Since you apply the `authenticate` middleware, the ID of the authenticated customer is stored in the `auth_context.actor_id` property of the request object.
...
Finally, send a `POST` request to the `/store/restock-susbcriptions` API route:

```bash
curl -X POST http://localhost:9000/store/restock-susbcriptions \
...
You'll receive a `201` response, indicating that the guest customer with email `customer@gmail.com` is now subscribed to restock notifications for the specified variant in the sales channels associated with the specified publishable API key.
    ...
    steps: [
      {
        ...
        description: "Retrieve all restock subscriptions using Query.",
        depth: 1,
...
The first step `useQueryGraphStep` is from Medusa's workflows. So, you'll only implement the remaining three steps.
...
In this step, you loop over the restock subscriptions and use the `hasOutOfStockLocations` utility to check whether a subscription's variant is out-of-stock in the sales channel's locations.
